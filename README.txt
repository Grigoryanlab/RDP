...
It is worth noting that our script can also compute just the underlying distance metrics (RMSDs, DOCKQ) with the -j flag. When tested against the DOCKQ script, is approximately 3.6 times faster in system time. This is despite the fact that in testing, DOCKQ.py was explicitly given which chains should be paired to each other, while atom\_match\_n\_RD p-value.py was only told the chains for the docking partners in the true structure, and compared all chains to each other to automatically match them. So this script may also prove useful for those who would like a slight increase in speed, or to analyze models with chains that share the same identifier, when running many DOCKQ calculations en masse.
